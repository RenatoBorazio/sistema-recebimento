...
def carregar_recebimentos():
    try:
        ...
        colunas_novas = {
            ...
            "Custo PC": 0.0, "Produto (SB1)": "", "Avisos": "", "Linha": 0, "Curva ABC": "-", "Ruptura": "Não"
        }
    ...
    for idx, row in df.iterrows():
        ...
        if cod_interno and not df_curva.empty and 'CODIGO' in df_curva.columns:
            ...
            if not m_curva.empty:
                curva_abc = str(m_curva['CURVA'].iloc[0]).strip().upper()
            ...
            if len(peds_multiplos) > 1:
                ...
                if linhas_split:
                    ...
                    for i, (p, q_raw) in enumerate(linhas_split):
                        ...
                        nova_linha['Curva ABC'] = row.get('Curva ABC', '-')
                ...
                df_base.at[real_idx, 'Curva ABC'] = row.get('Curva ABC', '-')
                ...
                for nf in notas_unicas:
                    ...
                    with st.expander(f"{status_tag} 🧾 NF: {nf} ({tipo_nf_atual}) | 🏷️ Fornec: {fornecedor} | 💰 R$ {v_total:,.2f}", expanded=False):
                        ...
                        if itens_com_divergencia:
                            ...
                            # --- Integração Curva e Ruptura no Texto ---
                            curvas_erros = df_erros['Curva ABC'].unique()
                            curvas_validas = [c for c in curvas_erros if c != "-" and pd.notna(c) and str(c).strip() != ""]
                            curvas_str = ", ".join(curvas_validas) if curvas_validas else "Não identificada"
                            
                            tem_ruptura = (df_erros['Ruptura'] == "Sim").any()
                            rupt_str = "COM RUPTURA" if tem_ruptura else "SEM RUPTURA"

                            texto_padrao = f"{fornecedor} | {filial}\n\n"
                            texto_padrao += f"NF {nf} - {tipo_nf_atual}\n"
                            texto_padrao += f"PC {pc_str}\n\n"
                            texto_padrao += f"⚠️ ATENÇÃO: Esta NF possui divergências em produtos da Curva [{curvas_str}] que estão {rupt_str} de estoque.\n\n"
                            texto_padrao += "Divergências identificadas:\n"
                            texto_padrao += "- " + "\n- ".join(lista_erros) + "\n\n"
